...
public class UserAction extends SuperAction {
	...
	public String usernick_check() {
		...
		if (userparam.length() < 4 && userparam.length() > 0) {
			ResponseUtil.write("<span id='ajax_usernick' class='alert_css'>4文字以上で入力してください</span>");
			return null;
		} else if (userparam.length() > 20) {
			ResponseUtil.write("<span id='ajax_usernick' class='alert_css'>20文字以下で入力してください</span>");
			return null;
		} else if (userparam.length() == 0) {
			ResponseUtil.write("<span id='ajax_usernick' class='alert_css'>入力してください</span>");
			return null;
		}
	...
	public String password_check() {
		...
		System.out.println("kyuuri" + userparam);
		if (userparam.length() < 4 && userparam.length() > 0) {
			ResponseUtil.write("<span id='ajax_password' class='alert_css'>4文字以上で入力してください</span>");
			return null;
		} else if (userparam.length() > 8) {
			ResponseUtil.write("<span id='ajax_password' class='alert_css'>8文字以下で入力してください</span>");
			return null;
		} else if (userparam.length() == 0) {
			ResponseUtil.write("<span id='ajax_password' class='alert_css'>入力してください</span>");
			return null;
		}
		if (!userparam.matches(passwordPattern)) {
			ResponseUtil.write("<span id='ajax_password' class='alert_css'>半角英数字で入力してください</span>");
			return null;
		}
